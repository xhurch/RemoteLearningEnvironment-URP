...
public class XhurchTeleport : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (nativityTransitionTime >= nativityTransitionThreshold && nativityIsTransitioning)
        {
            ...
            // Always hide the parent teleport container after transitioning to a new scene
            // since the player will have to interact with an object to enable the scene teleports again
            XhurchTeleportPoint teleportPoint = teleportingToMarker as XhurchTeleportPoint;
            var parentTeleportContainer = teleportPoint.transform.parent.gameObject;
            parentTeleportContainer.SetActive(false);
