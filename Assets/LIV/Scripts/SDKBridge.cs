...
namespace LIV.SDK.Unity
{
    public static class SDKBridge
    {
        ...
        static System.Action _injection_InputFrameHandler = null;
        ...
        {
            SDKInputFrame inputFrame = SDKInputFrame.empty;
            if (_injection_SDKInputFrame.active)
            {
                if (_injection_InputFrameHandler != null) _injection_InputFrameHandler.Invoke();
                inputFrame = _injection_SDKInputFrame.data;
            }
            else
            {
                IntPtr structPtr = GetCompositorChannelObject(8, Tag("INPFRAME"), UInt64.MaxValue);
                if (structPtr == IntPtr.Zero) return SDKInputFrame.empty;
                inputFrame = (SDKInputFrame)Marshal.PtrToStructure(structPtr, typeof(SDKInputFrame));
                _injection_SDKInputFrame.data = inputFrame;
            }
            return inputFrame;
        }
